...
public class AmountUtils {
    ...
    public static BigDecimal changeY2F(double yuan) {
        /**
         * 因为double类型双精度的问题 BigDecimal d1 = new BigDecimal(0.6);
         ...
         */
        BigDecimal fen = new BigDecimal(yuan).movePointRight(2).setScale(0,
    ...
    public static BigDecimal changeY2F(String yuan) {
        if (!yuan.matches(CURRENCY_YUAN_REGEX)) {
            throw new IllegalArgumentException("入参" + yuan
                    + "不满足单位'元'的格式,要求精确到小数点后两位");
        }
        /**
         * 通过传入double类型的字符串,会比直接传入double类型,更能得到目标结果 理由可以参看
         * {@code changeY2F(double)} 并且不需要设置小数点后保留位
         */
        BigDecimal fen = new BigDecimal(yuan).movePointRight(2);// .setScale(0,
                                                                // RoundingMode.DOWN);
        return fen;
